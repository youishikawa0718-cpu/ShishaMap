...
シーシャが好きで「近くのシーシャ店」をGoogleマップで検索することが多かったのですが、カフェやバーなどシーシャ専門店ではない店舗が大量にヒットしてしまい、目的の店を探すのに毎回手間がかかっていました。

**「シーシャ専門店だけを、Googleマップよりシンプルにわかりやすく探せるアプリ」** を自分で作ろうと思ったのが開発のきっかけです。
...
マップをスクロールするたびにAPIリクエストが発生すると、レスポンスが重くなるだけでなくAPI利用料金も増大します。**デバウンス処理**（一定時間操作が止まるまでリクエストを遅延）を実装し、不要なAPI呼び出しを抑制しました。加えてキャッシュ機構を導入し、同じエリアの再検索時にはローカルデータを優先して返すことで、UXとコストの両面を改善しています。

### 3. App Store審査対応（現在進行中）
